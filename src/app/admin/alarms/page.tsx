// Pfad: src\app\admin\alarms\page.tsx
...
export default function AlarmAdminPage() {
  ...
  const fetchTopics = async () => {
    try {
      const response = await fetch(`${basePath}/api/alarms/topics`);
      if (!response.ok) {
  ...
  const fetchRules = async () => {
    try {
      const response = await fetch(`${basePath}/api/alarms/rules`);
      if (!response.ok) {
  ...
  const handleEditRule = async (values: RuleData) => {
    try {
      const response = await fetch(`${basePath}/api/alarms/rules/${values.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteRule = async (id: number) => {
    try {
      const response = await fetch(`${basePath}/api/alarms/rules/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
